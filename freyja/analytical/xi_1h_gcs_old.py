...
@njit
def gaussian_pdf(vlos, sigma):
    return (1.0 / (np.sqrt(2.0 * np.pi) * sigma)) * np.exp(-0.5 * (vlos / sigma)**2)

@njit
def pdf_vlos_1h_cs_func_numba(vlos, sigma_vir, alpha_c=0.0, alpha_s=1.0):
    stddev = np.sqrt(alpha_c**2 + alpha_s**2) * sigma_vir
    return gaussian_pdf(vlos[..., np.newaxis], stddev)

@njit
def pdf_vlos_1h_ss_func_numba(vlos, sigma_vir, alpha_c=0.0, alpha_s=1.0):
    stddev = 1.41421356237 * alpha_s* sigma_vir
    return gaussian_pdf(vlos[..., np.newaxis], stddev)
...
class xi_1h_analy:
    ...
    def get_xiS_1h_cs(
        ...
        pdf_vlos = pdf_vlos_1h_cs_func_numba(
            vlos, 
            self.sigma_vir_halo, 
    ...
    def get_xiS_1h_ss(
        ...
        rr = np.sqrt(r_perp**2 + r_parallel_integrand**2)
        log10_r_fft = np.log10(self.fft.r)
        log10_rr = np.log10(rr)
        ...
        interp_func = interp1d(
            log10_r_fft,
            Akari,
            axis=0,  # interpolate over first axis of Akari (r), vectorized over mass bins
            bounds_error=False,
            fill_value=0.0,
        )
        Akari_integrand = interp_func(log10_rr)  # shape: (len(log10_rr), n_mass_bins)

        # vlos PDF
        vlos = (s_parallel - r_parallel_integrand) * np.sign(r_parallel_integrand)
        pdf_vlos = pdf_vlos_1h_ss_func_numba(
            vlos, 
            self.sigma_vir_halo, 
            alpha_s=self.HOD_params.v_bias_satellites,
        )
