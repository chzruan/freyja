...
def compute_xi_from_Pk(k_input, P_input, r_output, r_min = 0.1, r_max=150.0, smooth_xi=True):
    r"""
    ...
    k_interp = np.logspace(np.log10(k_input[0]), np.log10(k_input[-1]), 10000)
    ...
    if smooth_xi:
        ...
        r2xi_smooth = savgol_filter(spline_r2xi(r_output), window_length=11, polyorder=3)
